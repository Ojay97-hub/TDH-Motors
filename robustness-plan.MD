...
### 1. Admin Authorisation Privilege Escalation
...
- Enforce admin authorisation through one explicit source of truth, preferably either:
  - a Supabase `admin_users` table, or
...
- Replace aggressive content overwrite behaviour in seed scripts with safer defaults.
...
### 5. Sanity Fetch And Data Normalisation
...
- Harden car and merch normalisation so missing draft fields, null numbers, and empty arrays cannot crash pages.
- Keep schema validation as a first line of defence, but ensure runtime data normalisation remains defensive.
...
- Add visible error and retry behaviour for `InventoryNavigator` Sanity listener failures.
- Normalise `previewOrigin` in `sanity.config.ts` to avoid malformed draft-mode URLs from trailing slashes.
...
- Add a server-side admin authorisation helper, for example `requireAdmin()`.
...
- Unit-test car normalisation with missing draft fields:
  - missing `highlights`
  - missing images
  - null numeric fields
...
- Authorisation-test admin access:
...
- Manually verify:
  ...
  - draft-mode and cache revalidation behaviour still work after `safeSanityFetch` changes.
...
- Seed and migration scripts should default to non-destructive behaviour.
...
### Prompt 1: Admin Authorisation Privilege Escalation
We need to fix the Admin Authorisation Privilege Escalation.
